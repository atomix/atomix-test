...
def cluster_to_str(cluster):
    ...
    data = [['ID', 'NAME', 'VERSION', 'STATUS', 'PATH', 'IP', 'LOCAL PORT', 'PROFILER PORT'],]
    for node in cluster.nodes():
        profiler_port = node.profiler_port
        data.append([node.id, node.name, node.version, node.status, node.path, node.ip, node.local_port, profiler_port if profiler_port is not None else ''])
    return _create_table(data)
